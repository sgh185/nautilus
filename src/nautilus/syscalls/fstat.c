...
int sys_fstat(uint64_t fd, uint64_t statbuf_) {
  ...
  default: {
		DEBUG("WARNING: fstat is not properly implemented for non-std(in,out,err) file descriptors.");
    ret = nk_fs_fstat((struct nk_fs_open_file_state*)fd, (struct nk_fs_stat*)statbuf_);
	}
  }
  return ret;
}
